...
import { taskInstances, taskMasters } from "./tasks";
...
export const cardRewardConfigs = pgTable(
  "card_reward_configs",
  {
    ...
    approvalStatus: rewardApprovalStatusEnum("approvalStatus")
      .default("draft")
      .notNull(),
    ...
    createdBy: uuid("createdBy")
      .notNull()
      .references(() => users.id, { onUpdate: "no action" }),
  ...
  () => [
    check(
      ...
      sql`(("cardId" IS NOT NULL)::int + ("taskInstanceId" IS NOT NULL)::int + ("taskMasterId" IS NOT NULL)::int) = 1`,
    ),
  ],
);
...
export const cardRewardDeductions = pgTable("card_reward_deductions", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  configId: bigint("configId", { mode: "number" })
    .notNull()
...
export const cardRewardSnapshots = pgTable("card_reward_snapshots", {
  ...
  configId: bigint("configId", { mode: "number" })
    .notNull()
    .unique()
    .references(() => cardRewardConfigs.id, { onUpdate: "no action" }),
  ...
  snappedBonusAmount: decimal("snappedBonusAmount", {
    precision: 15,
    scale: 2,
  }),
...
export const cardRewardLogs = pgTable("card_reward_logs", {
  ...
  configId: bigint("configId", { mode: "number" })
    .notNull()
    .references(() => cardRewardConfigs.id, { onUpdate: "no action" }),
  deductionId: bigint("deductionId", { mode: "number" }).references(
    () => cardRewardDeductions.id,
...
export const cardRewardFinalizations = pgTable("card_reward_finalizations", {
  ...
  configId: bigint("configId", { mode: "number" })
    ...
    .unique()
    .references(() => cardRewardConfigs.id, { onUpdate: "no action" }),
  completionPercent: decimal("completionPercent", {
    precision: 5,
    scale: 2,
  }).notNull(),
  ...
  finalizedBy: uuid("finalizedBy")
    .notNull()
    .references(() => users.id, { onUpdate: "no action" }),
  finalizedAt: timestamp("finalizedAt", { precision: 6 })
    .defaultNow()
    .notNull(),
});

export const cardRewardConfigsRelations = relations(
  cardRewardConfigs,
  ({ one, many }) => ({
    card: one(cards, {
    ...
    taskMaster: one(taskMasters, {
      fields: [cardRewardConfigs.taskMasterId],
      references: [taskMasters.id],
      relationName: "rewardConfigsTaskMaster",
    }),
...
export const cardRewardDeductionsRelations = relations(
...
export const cardRewardSnapshotsRelations = relations(
  cardRewardSnapshots,
  ({ one }) => ({
    config: one(cardRewardConfigs, {
...
export const cardRewardFinalizationsRelations = relations(
